...
import { 
  ...
  Category as CategoryIcon, FilterList as FilterIcon, Download as DownloadIcon,
...
const UserTypesContainer = styled(Box)(({ theme }) => ({
...
export default function UserTypes() {
  ...
  // Filter userTypes based on userType
  ...
  // Calculate stats
  const totalTypes = userTypes.length;
  const activeTypes = userTypes.filter(type => type.isActive).length;
  const recentTypes = userTypes.filter(type => {
    const typeDate = new Date(type.createdAt);
    const weekAgo = new Date();
    weekAgo.setDate(weekAgo.getDate() - 7);
    return typeDate > weekAgo;
  }).length;
    ...
    <UserTypesContainer>
      {loading || userTypes[0] === 'empty'? <LoadingScreen caption='Loading...' fullScreen={false} /> : (
        <>
          {/* Page Header */}
          <PageHeader>
            <PageTitle>User Types Management</PageTitle>
            <PageSubtitle>Manage and monitor all user type categories in your system</PageSubtitle>
          </PageHeader>
                ...
                <CardContent sx={{ p: { xs: 2, sm: 3 }, pt: { xs: 1.5, sm: 2 } }}>
                  <Stack direction="row" alignItems="center" spacing={2}>
                    ...
                    <Box>
                      ...
                      <StatsValue color="#1976d2">{totalTypes}</StatsValue>
                    </Box>
                  </Stack>
                </CardContent>
              </StatsCard>
                ...
                <CardContent sx={{ p: { xs: 2, sm: 3 }, pt: { xs: 1.5, sm: 2 } }}>
                  <Stack direction="row" alignItems="center" spacing={2}>
                    ...
                    <Box>
                      ...
                      <StatsValue color="#4caf50">{activeTypes}</StatsValue>
                    </Box>
                  </Stack>
                </CardContent>
              </StatsCard>
                ...
                <CardContent sx={{ p: { xs: 2, sm: 3 }, pt: { xs: 1.5, sm: 2 } }}>
                  <Stack direction="row" alignItems="center" spacing={2}>
                    ...
                    <Box>
                      ...
                      <StatsValue color="#ff9800">{recentTypes}</StatsValue>
                    </Box>
                  </Stack>
                </CardContent>
              </StatsCard>
          ...
          <TableContainer >
              ...
              <Grid container spacing={3} alignItems="center">
                ...
                <Grid item xs={12} md={6}>
                  <Stack direction="row" spacing={2} justifyContent="flex-end" alignItems="center">
                    <Chip 
                      label={`${filteredTypes.length} types`} 
                      color="primary" 
                      variant="outlined"
                      sx={{ fontWeight: 600 }}
                    />
            ...
            <UserTypesTable userTypes={paginatedTypes} loadingRowId={loadingRowId} />
              ...
              <PaginationContainer>
                <Box>
                  <Typography variant="body2" color="text.secondary">
                    Showing {startIndex + 1} to {Math.min(endIndex, filteredTypes.length)} of {filteredTypes.length} user types
                  </Typography>
                </Box>
                <StyledPagination
                  count={totalPages}
