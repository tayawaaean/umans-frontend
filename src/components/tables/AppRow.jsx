...
const AppRow = ({ app, loadingRowId, animationDelay = 0 }) => {
    ...
    <StyledTableRow animationDelay={animationDelay}>
      ...
      <StyledTableCell align="center" sx={{ width: '8%' }}>
        {isEditing ? (
          ...
          <Tooltip title={app.isActive ? "Deactivate App" : "Activate App"}>
            <ActionButton 
              onClick={() => dispatch(updateApp({ id: app.id, data: { isActive: !app.isActive } }))}
              variant={app.isActive ? "save" : "cancel"}
            >
              {app.isActive ? <CheckCircle /> : <Cancel />}
            </ActionButton>
          </Tooltip>
        )}
      </StyledTableCell>
